...
export default function StopSelector({stop, setStop, stopsData}: {stop: Stop, setStop: (stop: Stop) => void, stopsData: Stop[]}) {
    ...
    return (
        <select 
            className="text-center p-2 w-auto border border-white rounded-lg opacity-90 text-white hover:border-orange-500 hover:text-orange-500 transition duration-100" 
            id="stop-selector" 
            name="stops" 
            onChange={(e) => setStop(getStopById(e.target.value))} 
            value={stop.id}
        >
            {stopsData.map((s) => (
